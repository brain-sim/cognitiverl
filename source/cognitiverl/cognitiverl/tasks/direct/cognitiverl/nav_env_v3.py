...
class NavEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        image_obs = self.camera.data.output["rgb"].permute(0, 3, 1, 2)
        ...
        image_obs = torch.nan_to_num(image_obs)
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for k, v in self.obs_buf.items():
            if k != "policy":
                del self.obs_buf[k]  # return observations, rewards, resets and extras
        return (
            self.obs_buf,
            self.reward_buf,
